...
function renderSetTableHeader(max_len) {
    const table_head = document.querySelector("#set-table thead");
...
export default function renderOneDArrayTable(start_step,last_step,all_variables) {
    ...
    //ヘッダーの設定
    renderSetTableHeader(max_len);

    const table_body = document.querySelector("#set-table tbody");
    //中身をリセット
    ...
    const union_keys = Array.from(
        new Set(
            all_variables.slice(start_step, last_step).flatMap(dict => dict["set"] ? Object.keys(dict["set"]) : [])
        )
    );

    //変数がないなら表示を消してパス
    if(union_keys.length <= 0){
        document.getElementById("set-div").style.display = "none";
        return;
    }else{
        document.getElementById("set-div").style.display = "block";
    }
    ...
    for(let i=0; i < union_keys.length; i++) {
        ...
        for(let j=start_step; j < last_step; j++) {
            ...
            if (all_variables[j]?.["set"]?.[union_keys[i]] == undefined) {
                continue;
            }
            ...
            //値を追加
            for(let k=0;k < all_variables[j]["set"][union_keys[i]].length; k++) {
                const value_cell = document.createElement("td");
                value_cell.textContent = all_variables[j]["set"][union_keys[i]][k];
                row.appendChild(value_cell);
            }

            //残ったのはN/Aを追加
            for(let k=all_variables[j]["set"][union_keys[i]].length;k < max_len; k++) {
                const value_cell = document.createElement("td");
